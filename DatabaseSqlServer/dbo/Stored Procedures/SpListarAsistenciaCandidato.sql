...
-- Minutos de Tardanza
-- EXEC [SpListarAsistenciaCandidato] 2017,10
CREATE PROC [dbo].[SpListarAsistenciaCandidato]
@Anho AS INT,
@Mes AS INT
AS
BEGIN
	SELECT	CAST(T0.Fecha AS DATETIME) + CAST('22:00:00' AS DATETIME) AS FechaHoraNocheInicial,
			...
			T0.Codigo,
			T2.ApellidoPaterno + ' ' + T2.ApellidoMaterno + ', ' + T2.Nombres AS Candidato,
			T0.Fecha,
			T0.Semana,
			T0.Anho,
	...
	INNER JOIN TbCandidato T2 ON T2.Codigo = T0.Codigo
							AND T2.Contratado = 0  
